...
const InputControlledComponent = ({
  ...
  const searchSpace = withSearchIcon ? 'pl-9' : '';

  return (
    <div className=" w-full">
      <span className="relative ">
        {withSearchIcon && (
          <Search className="absolute text-gray-6 top-[0px] left-[8px] right-3" />
        )}
        <input
          value={value}
          ref={register}
          className={`form-input w-full text-base text-gray-7 placeholder-gray-5 rounded ${sizeClasses} ${inputSpacing} ${inputClasses} disabled:bg-gray-3 ${searchSpace}`}
          disabled={isDisabled}
          name={name}
